...
public class TextSticker extends Sticker {
  ...
  public TextSticker(@NonNull Context context,boolean rotateX,boolean rotateY,Float rotationDegrees) {
    this(context, null);
...
//
//  public void rotateY(boolean rotateY){
//    this.rotateY = rotateY;
//  }

  //everything related to rotation of text will be done in here {In void draw function}
  @Override public void draw(@NonNull Canvas canvas) {
    Matrix matrix = getMatrix();
    canvas.save();
    canvas.concat(matrix);

    //applying drawable to the textSticker
    if (drawable != null) {
    ...
    if (textRect.width() == getWidth()) {
      ...
      //for rotation
      if (rotateY && rotateX && rotationDegrees != 0.0){
        // Apply perspective transformation (2.5D effect)
        if (rotationDegrees <50){
          matrix.reset();
          matrix.postSkew(rotationDegrees, rotationDegrees, 0, dy);
      ...
      else if (rotateX && rotationDegrees != 0.0){
        // Apply perspective transformation (2.5D effect)

        if (rotationDegrees < 50){
          matrix.reset();
          matrix.postSkew(-rotationDegrees,0, 0, dy);
     ...
     else if (rotateY && rotationDegrees != 0.0){

       if (rotationDegrees < 50){
         matrix.reset();
         matrix.postSkew(0, -rotationDegrees, 0, dy);
  ...
  public TextSticker setShadowColor(@ColorInt int shadowColor) {
  ...
  @NonNull public TextSticker resizeText() {
    ...
    textPaint.setShadowLayer(shadowRadius, 2, 2,shadowColor);
  ...
  protected int getTextHeightPixels(@NonNull CharSequence source, int availableWidthPixels, float textSizePixels) {
    ...
    // since api 23.
    StaticLayout staticLayout = new StaticLayout(source, textPaint, availableWidthPixels, Layout.Alignment.ALIGN_NORMAL,
