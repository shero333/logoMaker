...
public class StickerView extends FrameLayout {
    ...
    public StickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            borderPaint.setColor(a.getColor(R.styleable.StickerView_borderColor, ContextCompat.getColor(LogoMakerApp.appContext, R.color.theme_color)));
            borderPaint.setAlpha(a.getInteger(R.styleable.StickerView_borderAlpha, 200));
    ...
    protected void drawStickers(Canvas canvas) {
        ...
        //put the handling sticker in the start of the array
        for (int i = 0; i < stickers.size(); i++) {
            Sticker sticker = stickers.get(i);
            if (sticker == handlingSticker) {
                stickers.remove(sticker);
                stickers.add(sticker);
            }
        }

        //drawing the stickers on the view
        for (int i = 0; i < stickers.size(); i++) {
            Sticker sticker = stickers.get(i);
            if (sticker != null) {
                sticker.draw(canvas);
        ...
        if (handlingSticker != null && !locked && (showBorder || showIcons)) {
            ...
            //drawing handling sticker on the top of all the other stickers
    ...
    protected void onTouchUp(@NonNull MotionEvent event) {
        ...
        if (currentMode == ActionMode.DRAG && Math.abs(event.getX() - downX) < touchSlop &&
                Math.abs(event.getY() - downY) < touchSlop && handlingSticker != null) {
        ...
        if (currentMode == ActionMode.DRAG && (handlingSticker != null && !locked && (showBorder || showIcons))) {
            if (onStickerOperationListener != null) {
    ...
    private void rotateSticker(@Nullable Sticker sticker, @NonNull MotionEvent event) {
        if (sticker != null) {
    ...
    private void zoomSticker(@Nullable Sticker sticker, @NonNull MotionEvent event) {
        if (sticker != null) {
    ...
    public void remove(@Nullable Sticker sticker) {
    ...
    //customization by Muhammad Sheharyar {Moving the sticker from left to right and top to bottom}
    public Sticker moveStickerLeft(Float leftPosition) {

        if (handlingSticker != null) {
            moveMatrix.postTranslate(leftPosition, 0);
            handlingSticker.setMatrix(moveMatrix);
            if (constrained) {
                constrainSticker(handlingSticker);
            }
    ...
    public Sticker moveStickerRight(Float rightPosition) {

        if (handlingSticker != null) {
            moveMatrix.postTranslate(rightPosition, 0);
            handlingSticker.setMatrix(moveMatrix);
            if (constrained) {
                constrainSticker(handlingSticker);
            }
    ...
    public Sticker moveStickerUp(Float upPosition) {

        if (handlingSticker != null) {
            moveMatrix.postTranslate(0, upPosition);
            handlingSticker.setMatrix(moveMatrix);
            if (constrained) {
                constrainSticker(handlingSticker);
            }
    ...
    public Sticker moveStickerDown(Float downPosition) {

        if (handlingSticker != null) {
            moveMatrix.postTranslate(0, downPosition);
            handlingSticker.setMatrix(moveMatrix);
            if (constrained) {
                constrainSticker(handlingSticker);
            }
