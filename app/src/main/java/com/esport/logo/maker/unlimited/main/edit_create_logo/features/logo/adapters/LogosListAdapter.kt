package com.esport.logo.maker.unlimited.main.edit_create_logo.features.logo.adapters
...
class LogosListAdapter(private var clickedImage: ItemClickedImages) :
    RecyclerView.Adapter<LogosListAdapter.LogosListViewHolder>() {

    private var logoImageList:ArrayList<Image> = ArrayList()
    private var selectedItemPosition = 0

    fun setLogosList(logoImageList:ArrayList<Image>){
        this.logoImageList = logoImageList
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LogosListViewHolder {

        val view = LayoutInflater.from(parent.context).inflate(R.layout.background_image_list_itemview,parent,false)
        return LogosListViewHolder(view)
    }
    ...
    override fun onBindViewHolder(holder: LogosListViewHolder, position: Int) {
    ...
    fun changeItemBackground(holder: LogosListViewHolder, isSelected: Boolean) {
        if (isSelected) {
    ...
    class LogosListViewHolder(itemView:View) : RecyclerView.ViewHolder(itemView){
